...
这最后一章（除了结论和线上章节）将看起来有点不同。它包含更多代码，且比之前章节的少了些单调。我们会介绍一些新的 Python 关键字及没有讨论过的库。这一章节对你来说意味着是一个重要探索项目的开始。我们将从零开始实现一些 fastai 的关键部分和PyTorch API，你学习从无到有创建而不是我们在<第十七章：基础神经网络>中开发的组件！ 在这里关键的目标是以开发你自己的 `Learner` 类为结束，和一些足以能够在 Imagenette 上训练一个模型的回调，包括我们学习过的每个关键技术的例子。创建 `Learner` 的过程中，我们会创建我们自己版本的 `Module` ，`Parameter` 和并行 `DataLoader` ，所以你对那些 PyTorch 类做了什么会有一个很好的理解。
...
这一章节末尾的练习题是特别重要的。此处我们会指给你一些你可以获取的一些有趣的方向，利用这一章作为你的起点。我们建议你在你的计算机是学习本章节，并做一些实验，网络搜索，及另外诸如你需要理解这是怎么回事。在本书的其余部分你已经积累了技能和专业只是，所以我们认为你将会做得很好！
...
查看 `untar_data` 的原头，看它是如何工作的。在这里我们会使用它来访问用于本章的 Imagenette 160-像素版本：
...
或我们能够只是用 Python 的标准库 `glob` 做同样的事情：
...
如果你看了 `get_image_files` 的源代码，你会看到它使用了 Python 的 `os.walk`；相比 `glob` 它更快且更具有柔性，所以一定要试试它。

We can open an image with the Python Imaging Library's `Image` class:

我们可以用 Python 图像库 `Image` 类打开一张图像：
...
这将作为我们自变量的基础。我们的因变量，可以使用来自 `pathlib` 的 `Path.parent`。首先我们需要我们的词汇表：
...
认识到我们能够用如此少的代码实现 fastai 的 `Learner` 所有关键点是是非美妙的！现在让我们添加一些学习率调度。
...
如果我们要获得好的结果，我们会相要一个学习率查找器和 1 周期训练。这些都是*退火* 回调---即，当我们训练时，他们逐渐的改变超参。下面是 `LRfinder`：
...
上述代码展示了，我们如何使用 `CancelFitException`，它自身是一个空类，只用来表示例外类型。你能够在 `Learner` 中看到被捕捉的这个例外（你应该自己添加并测试 `CancelBatchException`，`CancelEpochException` 等）通过添加 `CancelFitException` 到我们的回调列表，我们实验一下：
...
<img src="/Users/Y.H/Library/Application Support/typora-user-images/image-20211003153116434.png" alt="image-20211003153116434" style="zoom:45%;" />  [12/74 00:02<00:12]
...
让我们先拟合一下并看效果如何（我们没有在本书中显示所有的输出，尝试在 notebook 中看看结果）：
...
最后，我们将检查学习率是否遵循我们定义的计划（如你所见，在这里我没有使用余弦退火）：
...
通过在本章重新实现，我们已经探索了 fastai 库的关键概念的实现。因为这几乎是全部代码，你应该在本书的网站上查找相应的 notebook 并一定尝试用它做一下实验。现在你知道了它是如何创建的了，那下一步，一定要看 fastai 文档的中级和高级教程，学习如何定制这个库的每一小部分。
...
> 提示：实验：对于这里的这些问题，要求你解释这些函数或类是什么，你也应该完成你自己的代码实验。
...
1. 什么是 `glob`？
2. 你如何用 Python 图像库打开一张图像？
3. `L.map` 做了什么？
4. `Self` 做了什么：
5. 什么是 `L.val2idx`？
6. 你需要什么方法来实现创建你自己的 `Dataset`？
7. 当你打开一张来自 Imagenette 的图像时，我们为什么要调用 `convert`？
8. `~` 做了什么？它是如何用来分割训练集和验证集？
9. `~` 与 `L` 或 `Tensor` 类配合工作吗？那 NumPy 数组、Python 列表或 pandas 的 DataFrames 呢？
10. 什么是 `ProcessPoolExecutor`？
11. `L.range(self.ds)` 是如何工作的？
12. 什么是 `__iter__`？
13. 什么是 `first`？
14. 什么是 `permute`？为什么需要它？
15. 什么是递归函数？它是如何帮助我们定义 `parameters` 方法的？
16. 编写一个返回斐波纳契数列头 20 个数据项的递归函数。
17. 什么是 `super`？
18. 为什么 `Module` 的子类需要覆盖 `forward` 而不是定义`__call__`？
19. 在 `ConvLayer` 中，为什么 `init` 依赖 `act` ？
20. 为什么 `Sequential` 需要调用 `register_modules`？
21. 编写一个钩子，打印输出每层激活的形状。
22. 什么是 “LogSumExp”？
23. 为什么 `log_softmax` 是有用的？
24. 什么是 `GetAttr`？它对回调有什么帮助？
25. 重新实现本章节中的一个回调，不要继承 `Callback` 或 `GetAttr`。
26.  `Learner.__call__` 做了什么？
27. 什么是 `getattr`？（注意它与 `GetAttr` 的差异！）
28. 在 `fit` 中为什么有一个 `try` 代码块？
29. 为什么我们要检查 `one_batch` 中的 `model.training` ？
30. 什么是 `store_attr`？
31.  `TrackResults.before_epoch` 的用途是什么？
32. `model.cuda` 做了什么？它是如何工作的？
33. 为什么我们需要检查 `LRFinder` 和 `OneCycle` 中的`model.training`？
34. 在 `OneCycle` 中使用余弦退火。
...
1. 从零开始编写 `resnet18`（按需参照<第十四章 残差网络>），并用本章的 `Learner` 训练它。
2. 从零开始实现一个批次归一化层并在你的 `resnet18` 中使用它。
3. 编写一个 Mixup 回调，在本章中使用。
4. 给 SGD 添加动量。
5. 选一些出自 fastai（或其他任何库）你感兴趣的特性，在本章中实现它们。
6. 选一篇在 fastai 或 PyTorch 中一学没有实现的研究论文，并在本章中实现它。
   - 把它移植到 fastai。
   - 向 fastai 提交一个 pull 请求，或创建你自己的扩展模块并发布它。
   - 提示：你会发现使用 [`nbdev`](https://nbdev.fast.ai/)有帮于创建并发布你的包。