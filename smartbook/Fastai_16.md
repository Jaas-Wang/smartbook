...
我们需要从一个非常好的灵活的基础库开始，来建立我们的加速SGD技巧。之前没有库提供了像fastai这样的基础库，但fastai开发期间，我们意识到我们在学术文献中看到的所有优化器改良都可以用*优化器回调*来处理。这里有我们可以在优化器中组装、混合和比对来创建优化器`step`的一小部分代码。它们通过fastai轻权重`Optimizer`类所调用。下面是我们在本书中所使用的两个关键方法的`Optimizer`中的定义：
...
正如我们看学习过的，当从零开始训练一个MNIST模型时，`zero_grad`只是遍历模型的参数和设置梯度为零。它也调用移除了历史梯度计算的`detach_`，因为`zero_grad`之后就不需要它了。
...
更有意思的方法是`setp`，它遍历了回调（`cbs`）并调用它们来更新参数（如果`cb`有任何返回，`_update`函数就只调用`state.update`）。正如你所看到的`Optimizer`实际上它自己不做任何SGD步骤。让我们看一下我们如何能够添加SGD到`Optimizer`。
...
下面是一个做单SGD步骤的优化器回调，通过用`-lr`乘以梯度并加到参数上（当PyTorch中的`Tensor.add_`被传递了两个参数时，它们先相乘再相加）：
...
我们可以使用`cbs`参数把它传给`Optimier`；我们需要使用`partial`，因为稍后`Learner`会调用这个函数来创建我们的优化器
...
它可以运行！那么在fastai中我们如何从零开始创建SGD呢。现在让我们学习一下“动量”是什么。
...
正如在<第四章：mnist基础>中所描述的，SGD可以被认为是站在山顶上，并通过及时在每个点上沿着斜率最大的方向迈出一步的方式一路向下。但是如果我们有一个球滚下山会怎样呢？在每个给定的点上，它不会完全跟随梯度的方向，因为它有*动量*。一个有大量动量的秋（例如，一个很重的求）会略过小的突起和坑洼，且更有可能到达凹凸不平山的底部。从另一个角度说，一个乒乓球会卡在每个小的缝隙中。
...
所以我们如何能够应用这个想法到SGD上呢？我们可以使用移动平均，来代替当前仅有的梯度值，以得到我们的步进：
...
这里的`beta`是我们选择用来定义动量是多少的某个数值。如果`beta`是 0 ，第一个等式就变成了`weight.avg = weight.grad`，所以我们以普通的SGD而告终。所以如果这是一个接近 1 的值，那么主要的方向选择是之前步进的平均值。（如果你做了一些统计，你可以认为第一个等式是一个*指数加权移动平均*，它普通用于降噪数据和获得潜在趋势。）
...
注意我们正在编写的`weight.avg`以强调我们需要来存储模型的每个参数的移动平均的事实（他们都有他们自己独立的移动平均）。
...
在图<img_momentum>中展示了一个对于单一参数噪音数据的例子，红色的是动量曲线图，蓝色的是参数的梯度图。梯度先增加，然后减少，动量很好的跟随了梯度的整体趋势，没有受到噪音的太多影响。
...
如果损失函数有狭窄的裂缝我需要横渡过云，它会做的尤其的好：普通的SGD会让我们从一侧弹跳到另一侧，当具有动量的SGD的时候会把那些裂缝平均，从而平顺的滚下侧面。参数`beta`决定了我们所使用动量的力度：用一个小的`beta`我们保持接近于实际的梯度值，然而用一个高的`beta`我们几乎朝着梯度的平均值方向前进，且在梯度做出趋势移动改变都会需要一段时间。
...
一个大的`beta`，我们可能会丢失已经改变了方向的梯度和滚过小区域的极小值。这是期望的副作用：直觉上来说，当我们给模型展示一个新的输入，它看起来会像训练集的某物，但是又不是*绝对的*像。这表示它们会对应于损失函数中接近我们训练结束时得到的最小值的那个点，但不是*完全*在那个最小值上。所以，我们宁愿训练以一个广泛的最小值而告终，在 这些点附近有近似的相同损失（或如果你愿意，一个损失尽可能平坦的点）。在<img_betas>中，展示了因为我们改变`beta` 图<img_momentum>中的图表是如何变化的。
...
在这些例子中我们能够看到`beta`太高会导致整个剃度的变化被忽略。在SGD中用动量，`beta`的值通常使用的是 0.9。
...
`fit_one_cycle`默认`beta`开始值是 0.95 ，逐步把它的值调整到 0.85 ，然后逐步调整它的值在训练结束时又回到 0.95。让我们看一下我们带有动量的训练是如何添加到普通的SGD上的。
...
为了增加动量到我们的优化器上，我们首先需要保持跟踪移动平均梯度，我们通用用其它回调来做这个事情。当一个优化器回调返回了一个`dict`时，它是被用于更新优化器的状态并传递返回到下一步上的优化器。所以这个回调会持续跟踪名为`grad_avg`参数中的梯度平均：
...
我们只需要在我们的步进函数中用`grad_avg`替换`p.grad.data`来使用它：
...
`Learner`会自动的安排`mom`和`lr`，所以`fit_one_cycle`甚至会用我们自己定义的`Optimizer`来工作：
...
RMSProp是另一个SGD变种，是由杰弗里·辛顿在它的Coursera课堂["机器学习神经网络"](http://www.cs.toronto.edu/~tijmen/csc321/slides/lecture_slides_lec6.pdf)的讲义6e中介绍的。它与SGD的主要区别是使用了自适应学习率：与每个参数使用相同的学习率相反，通过一个全局学习率控制每个参数会获得它自己的特定学习率。由此我们能够对于需要改变很多的权重通过给定的更高学习率加速训练，同时对于足够好的那些权重使用更低的学习率。
...
我们如何决定那些参数应该有高的学习率及哪些参数不应该？我们可以看一下梯度来获得思路。如果一个参数的梯度已经暂时接近了零，那个参数就会需要高的学习率，因为其损失是平的。换句话说，如果梯度到处都是，我们可能应该小心并选取低的学习以避免发散。我们不能只是平均梯度来看他们是否有些改变，因为大的正数和大的负数的平均接近于零。因为，我们可以使用常用的技巧对两者取绝对值或平方值（且然后求平均数的平方根）。
...
同样的，为了决定噪音之后的整体趋势，我们会使用移动平均--特定的梯度平方移动平均。然后，我们会通过使用当前梯度（这是方向）除以这个移动平均的平方根来更新相应的权重（如果这是低的，生效的学习率会更高。如果它是高的，生效的学习率会更低）：
...
为了数值稳定加了`eps`（*epsilon*）（通常设置为1e-8），`alpha`的默认值通常是 0.99 。

We can add this to `Optimizer` by doing much the same thing we did for `avg_grad`, but with an extra `**2`:

我们能够通过对`avg_grad`做很多同样的事情，并添加它到`Optimizer`，但是多了一个额外的`**2`：
...
- `after_loss`：损失已经被计算之后，但是反向传播传送之前调用。它可以被用于给损失增加处罚（例如，在RNN训练中的AR or TAR）。
- `after_backward`：反向传播传递之后，但是参数更新之前调用。它可以用于对所述更新之前的梯度做出改变（例如，通过梯度裁剪）。
